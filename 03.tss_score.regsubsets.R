library(leaps)
disease=read.table("/data/Figure 4/regsubsets.tss_score.early_PE.train_valid.txt",header=T,sep="\t",check.names=F,row.names=1)
control=read.table("/data/Figure 4/regsubsets.tss_score.control.train_valid.txt",header=T,sep="\t",check.names=F,row.names=1)
...
pdf("/results/Figure 4/03.tss_score.regsubsets.EPE.pdf", height=3.4, width=3.6)
plot(mean.cv.errors, type='p',pch=20)
dev.off()

reg.best <- regsubsets(group ~ ., data=data_merge, nvmax=20)
coef(reg.best, id=best.id)
write.table(coef(reg.best, id=best.id),file="/results/Figure 4/03.tss_score.regsubsets.EPE.txt",sep="\t")

...
library(ggplot2) 
library(openxlsx) 
data=read.xlsx("/data/Extended Data Fig 6/05.model_performance.LPE.cfDNA.xlsx",sheet=1)
...
pdf("/results/Extended Data Fig 6/05.model_performance.LPE.cfDNA.AUC.pdf", width=2.9, height=1.4)
...
p1 <- ggplot() +
  ...
  theme(axis.ticks = element_blank(),axis.text.y=element_text(size=6),axis.text.x=element_text(margin=margin(5,5,-1,5),size=6,angle=10),axis.title=element_text(size=6))+
  theme(legend.text=element_text(size=6), plot.title = element_text(hjust = 0.5), legend.position="right", legend.title = element_blank())
pdf("/results/Extended Data Fig 6/05.model_performance.LPE.cfDNA.Sensitivity.pdf", width=2.9, height=1.4)
grid.arrange(p1 ,ncol=1, nrow=1)
dev.off()
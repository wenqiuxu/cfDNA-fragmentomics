...
data=read.xlsx("/data/Extended Data Fig 1/Extended_data_Figure1.xlsx",sheet=1)
data=na.omit(data)
...
p_tss_score<-compare_means(tss_score~ TPM, data = data, group.by = "reads_number",method="kruskal.test")
...
scale_y_continuous(sec.axis = sec_axis(~.*1550, name = '-log10(p.adjust)'))+
geom_point(data = p_tss_score, aes(x=factor(reads_number),y=-log10(p.adj)/1550), colour="#Cd0000",size=0.5)+
geom_line(data =  p_tss_score, aes(x=factor(reads_number),y=-log10(p.adj)/1550,group =1),colour="#Cd0000",cex=0.6)

pdf("/results/Extended Data Fig 1/Extended_data_Figure1b.pdf", height=1.25, width=2.1)
grid.arrange(p1)
dev.off()

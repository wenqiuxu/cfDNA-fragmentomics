...
##gini
disease=read.table("/data/Figure 4/shap.gini.early_PE.train_valid.txt",header=T,sep="\t",check.names=F,row.names=1)
control=read.table("/data/Figure 4/shap.gini.control.train_valid.txt",header=T,sep="\t",check.names=F,row.names=1)
...
pdf("/results/Figure 4/04.shap.gini.EPE.rf.pdf", height=1.5, width=2.5)

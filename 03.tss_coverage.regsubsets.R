library(leaps)
disease=read.table("/data/Extended Data Fig 6/regsubsets.tss_coverage.late_PE.train_valid.txt",header=T,sep="\t",check.names=F,row.names=1)
control=read.table("/data/Extended Data Fig 6/regsubsets.tss_coverage.control.train_valid.txt",header=T,sep="\t",check.names=F,row.names=1)
...
pdf("/results/Extended Data Fig 6/03.tss_coverage.regsubsets.LPE.pdf", height=3.4, width=3.6)
plot(mean.cv.errors, type='p',pch=20)
dev.off()

reg.best <- regsubsets(group ~ ., data=data_merge, nvmax=20)
coef(reg.best, id=best.id)
write.table(coef(reg.best, id=best.id),file="/results/Extended Data Fig 6/03.tss_coverage.regsubsets.LPE.txt",sep="\t")

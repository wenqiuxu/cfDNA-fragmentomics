...
here_list = sys.argv
##input features
gene_list_file = pd.read_table("/data/Figure 4/boruta.tss_coverage.features.txt", index_col=0, header = None)
gene_list = gene_list_file.index.tolist()

#input matrix
train_disease = pd.read_table("/data/Figure 4/boruta.tss_coverage.early_PE.train_valid.txt", index_col=0).dropna()
train_control = pd.read_table("/data/Figure 4/boruta.tss_coverage.control.train_valid.txt", index_col=0).dropna()
...
boruta_ranking_fre.to_csv('/results/Figure 4/boruta_result.tss_coverage.EPE.txt', sep='\t')

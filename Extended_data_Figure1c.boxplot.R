...
data=read.xlsx("/data/Extended Data Fig 1/Extended_data_Figure1.xlsx",sheet=2)
data=na.omit(data)
data1=data[data$tss_score>0.01,]
colourCount = length(unique(data1$TPM)) 
getPalette = colorRampPalette(brewer.pal(11, "RdBu"))
p_pos <- max(data1$tss_score)-0.1

p1 <- ggplot(data=data1,aes(x=factor(TPM), y=tss_score,fill=factor(TPM)))+
...
stat_compare_means(label = "p.format",  method="kruskal.test", size=0.6, tip.length=0.03, label.y=1.9)+
labs(fill="TPM in placenta")+
theme(legend.position = 'bottom')+
 coord_cartesian(ylim =c(0,2))


pdf("/results/Extended Data Fig 1/Extended_data_Figure1c.pdf", height=1.85, width=1.7)

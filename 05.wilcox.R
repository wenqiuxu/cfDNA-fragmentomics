...
disease=read.table("/data/GRCh38_RNA.tss_coverage.early_PE.train_valid.txt",header=T,sep="\t")
control=read.table("/data/GRCh38_RNA.tss_coverage.control.train_valid.txt",header=T,sep="\t")
...
DE_TSSs <- function(disease,control){
  ...
  for (j in 1:1000){
    ...
    deg=d$DEG
    write.table(d$DEG,file=paste("/result/differential_analyses/EPE/GRCh38_RNA.tss_coverage_DEG",j,"txt",sep="."),col.names=T,row.names=F,quote=F,sep="\t")
  }

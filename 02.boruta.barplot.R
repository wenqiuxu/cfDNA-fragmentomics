...
library(Rmisc)
## tss coverage
data=read.xlsx("/data/Extended Data Fig 6/feature_selection.LPE.xlsx",sheet=1)
...
pdf("/results/Extended Data Fig 6/01.boruta.tss_coverage.barplot.pdf", height=1.6, width=2.2)
grid.arrange(p1, nrow=1, ncol=1)
dev.off()

## tss score
data=read.xlsx("/data/Extended Data Fig 6/feature_selection.LPE.xlsx",sheet=2)
# lock in factor level order 
data$ID <- factor(data$ID , levels = data$ID )
data$ranking<-round(data$ranking,4)
p1 <- ggplot(data = data, aes(x=ID, y=ranking, color="#1F4527")) +
  ...
  labs(colour="group")+
  theme(legend.text=element_text(size=6),legend.title=element_blank(),legend.key.size=unit(0.3,"cm"))+
  coord_cartesian(ylim =c(0.23,1.0))
pdf("/results/Extended Data Fig 6/01.boruta.tss_score.barplot.pdf", height=1.6, width=2.2)
grid.arrange(p1, nrow=1, ncol=1)
dev.off()

##gini
data=read.xlsx("/data/Extended Data Fig 6/feature_selection.LPE.xlsx",sheet=3)
# lock in factor level order 
data$ID <- factor(data$ID , levels = data$ID )
data$ranking<-round(data$ranking,4)
p1 <- ggplot(data = data, aes(x=ID, y=ranking, color="#1F4527")) +
  ...
  labs(colour="group")+
  theme(legend.text=element_text(size=6),legend.title=element_blank(),legend.key.size=unit(0.3,"cm"))+
  coord_cartesian(ylim =c(0.81,1.0))
pdf("/results/Extended Data Fig 6/01.boruta.gini.barplot.pdf", height=1.6, width=2.2)
grid.arrange(p1, nrow=1, ncol=1)
dev.off()
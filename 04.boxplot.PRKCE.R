...
library(Rmisc)
##TSS coverage
data=read.table("/data/Extended Data Fig 4/boxplot.tss_coverage.PRKCE.txt",header=TRUE,sep="\t",check.names = F)
...
pdf("/results/Extended Data Fig 4/boxplot.tss_coverage.PRKCE.pdf", height=1.2, width=1)
grid.arrange(p1,  ncol=1)
dev.off()


##gini PRKCE
data=read.table("/data/Extended Data Fig 4/boxplot.gini.PRKCE.txt",header=TRUE,sep="\t",check.names = F)
cols = c("#9cb0c3","#e9b383")
...
p1 <- ggplot(data=data,aes(x=group, y=gini, fill=group))+
  ...
  theme(legend.position="None")+
  stat_compare_means(label = "p.format", size=2, tip.length=0.03, label.y=c(1.2), label.x=c(1.4))
pdf("/results/Extended Data Fig 4/boxplot.gini.PRKCE.pdf", height=1.2, width=1)
grid.arrange(p1,  ncol=1)
dev.off()

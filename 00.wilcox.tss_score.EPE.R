...
disease=read.table("/data/GRCh38_RNA.tss_score.early_PE.train_valid.txt",header=T,sep="\t")
control=read.table("/data/GRCh38_RNA.tss_score.control.train_valid.txt",header=T,sep="\t")
...
DE_TSSs <- function(disease,control){
  ...
  for (j in 1:1000){
    ...
    deg=d$DEG
    write.table(d$DEG,file=paste("/results/Extended Data Fig 3/EPE/GRCh38_RNA.tss_score_DEG",j,"txt",sep="."),col.names=T,row.names=F,quote=F,sep="\t")
  }

...
data=read.xlsx("/data/Extended Data Fig 1/Extended_data_Figure1.xlsx",sheet=6)
data=na.omit(data)
...
df2_count <- summarySE(data1, measurevar="gini",groupvars=c("reads_number","TPM"))
p_gini<-compare_means(gini~ TPM, data = data1, group.by = "reads_number",method="kruskal.test")
...
scale_y_continuous(sec.axis = sec_axis(~.*50000, name = '-log10(p.adjust)'))+
geom_point(data = p_gini, aes(x=factor(reads_number),y=-log10(p.adj)/50000), colour="#Cd0000",size=0.5)+
geom_line(data =  p_gini, aes(x=factor(reads_number),y=-log10(p.adj)/50000,group =1),colour="#Cd0000",cex=0.6)

pdf("/results/Extended Data Fig 1/Extended_data_Figure1g.pdf", height=1.25, width=2.1)
grid.arrange(p1)
dev.off()

...
data=read.xlsx("/data/Extended Data Fig 1/Extended_data_Figure1.xlsx",sheet=4)
data=na.omit(data)
...
p_pos <- max(data1$tss_coveage)-0.1
...
stat_compare_means(label = "p.format",  method="kruskal.test", size=3, tip.length=0.03, label.y=0.6)+
labs(fill="TPM in placenta")+
theme(legend.position = 'bottom')+
 coord_cartesian(ylim =c(0,0.7))

pdf("/results/Extended Data Fig 1/Extended_data_Figure1e.pdf", height=1.85, width=1.7)

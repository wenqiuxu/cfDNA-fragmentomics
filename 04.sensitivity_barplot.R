...
##EPE 16w
data=read.xlsx("/data/Extended Data Fig 10/performance.xlsx",sheet = 1)
...
pdf("/results/Extended Data Fig 10/model_performance.Sensitivity.16.EPE.pdf", width=6.6, height=1.4)
grid.arrange(grobs=plotlist,  ncol=3)
dev.off()


##LPE 16
data=read.xlsx("/data/Extended Data Fig 10/performance.xlsx",sheet = 2)
data$Features=factor(data$Features, levels=c('MFs','cfDNA fragmentomics','combined'))
...
pdf("/results/Extended Data Fig 10/model_performance.Sensitivity.16.LPE.pdf", width=6.6, height=1.4)
grid.arrange(grobs=plotlist,  ncol=3)
dev.off()

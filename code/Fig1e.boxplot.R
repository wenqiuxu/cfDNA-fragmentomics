...
data=read.xlsx("/data/Figure 1/Figure1.xlsx",sheet=4)
data=na.omit(data)
...
stat_compare_means(label = "p.format",  method="kruskal.test", size=2, tip.length=0.03, label.y=1.75)+
labs(fill="TPM in placenta")+
theme(legend.position = 'bottom')+
 coord_cartesian(ylim =c(0,1.85))


pdf("/results/Figure 1/Figure1e.pdf", height=1.85, width=1.7)

...
disease=read.table("/data/GRCh38_RNA.tss_coverage.late_PE.train_valid.txt",header=T,sep="\t")
...
DE_TSSs <- function(disease,control){
  ...
  for (j in 1:1000){
    ...
    deg=d$DEG
    write.table(d$DEG,file=paste("/results/Figure 3/LPE/GRCh38_RNA.tss_coverage_DEG",j,"txt",sep="."),col.names=T,row.names=F,quote=F,sep="\t")
  }

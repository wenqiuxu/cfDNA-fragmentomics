...
data=read.xlsx("/data/Figure 1/Figure1.xlsx",sheet=8)
data=na.omit(data)
...
scale_y_continuous(sec.axis = sec_axis(~.*60000, name = '-log10(p.adjust)'))+
geom_point(data = p_gini, aes(x=factor(reads_number),y=-log10(p.adj)/60000), colour="#Cd0000",size=0.5)+
geom_line(data =  p_gini, aes(x=factor(reads_number),y=-log10(p.adj)/60000,group =1),colour="#Cd0000",cex=0.7)

pdf("/results/Figure 1/Figure1i.pdf", height=1.21, width=2.2)
grid.arrange(p1)
dev.off()

...
here_list = sys.argv
##input features
gene_list_file = pd.read_table("/data/Figure sourcedata and code/Figure 4/boruta.tss_coverage.features.txt", index_col=0, header = None)
gene_list = gene_list_file.index.tolist()

#input matrix
train_disease = pd.read_table("/data/Figure sourcedata and code/Figure 4/boruta.tss_coverage.early_PE.train_valid.txt", index_col=0).dropna()
train_control = pd.read_table("/data/Figure sourcedata and code/Figure 4/boruta.tss_coverage.control.train_valid.txt", index_col=0).dropna()
...
###initialize Boruta
forest = RandomForestRegressor(n_jobs = -1, max_depth = 189, min_samples_split=5, min_samples_leaf=3, n_estimators=200, random_state=42)
...
boruta_ranking_fre.to_csv('boruta_result.tss_coverage.EPE.txt', sep='\t')

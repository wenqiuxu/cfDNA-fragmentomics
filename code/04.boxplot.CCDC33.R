...
library(Rmisc)
##TSS coverage
data=read.table("/data/Figure 3/boxplot.tss_coverage.CCDC33.txt",header=TRUE,sep="\t",check.names = F)
...
pdf("/results/Figure 3/boxplot.tss_coverage.CCDC33.pdf", height=1.2, width=1)
grid.arrange(p1,  ncol=1)
dev.off()

##Gini
data=read.table("/data/Figure 3/boxplot.gini.CCDC33.txt",header=TRUE,sep="\t",check.names = F)
cols = c("#9cb0c3","#e9b383")
...
pdf("/results/Figure 3/boxplot.gini.CCDC33.pdf", height=1.2, width=1)
grid.arrange(p1,  ncol=1)
dev.off()
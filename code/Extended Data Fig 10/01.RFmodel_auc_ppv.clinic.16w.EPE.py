...
#input feature
gene_list_file = pd.read_table("/data/Extended Data Fig 10/RF.clinic.features.txt", index_col=0, header = None)
gene_list = gene_list_file.index.tolist()

#input matrix
train_disease = pd.read_table("/data/model_matrix_EPE/RF_EPE.16w.H1.early_PE.txt", index_col=0).dropna()
val_disease = pd.read_table("/data/model_matrix_EPE/RF_EPE.16w.H2.early_PE.txt", index_col=0).dropna()
test1_disease = pd.read_table("/data/model_matrix_EPE/RF_EPE.16w.H3.early_PE.txt", index_col=0).dropna()
test2_disease = pd.read_table("/data/model_matrix_EPE/RF_EPE.16w.H4.early_PE.txt", index_col=0).dropna()

train_control = pd.read_table("/data/model_matrix_EPE/RF_EPE.16w.H1.control.txt", index_col=0).dropna()
val_control = pd.read_table("/data/model_matrix_EPE/RF_EPE.16w.H2.control.txt", index_col=0).dropna()
test1_control = pd.read_table("/data/model_matrix_EPE/RF_EPE.16w.H3.control.txt", index_col=0).dropna()
test2_control = pd.read_table("/data/model_matrix_EPE/RF_EPE.16w.H4.control.txt", index_col=0).dropna()
...
final_predit = {'criterion': 'entropy', 'max_depth': 9, 'min_samples_leaf': 20, 'n_estimators': 200}
...
def bootstrap_auc(clf, X_matrix, y_matrix, nsamples=1000):
  ...
  for b in range(nsamples):
    ...
    for cutoff in np.linspace(0.2,0.5,1001):
...
model_i_str=str(i)
f = open ('/results/Extended Data Fig 10/EPE_clinic_16/model/EPE_clinic_16.dat','wb')
pickle.dump(clf,f)
f.close()

###plot AUC
filename1='/results/Extended Data Fig 10/EPE_clinic_16/auc/AUC_1.pdf'
#画图
plt.figure()
lw = 2
...
filename2='/results/Extended Data Fig 10/EPE_clinic_16/ppv/train.PPV_CI.csv'
f = open(filename2,'a')
f.write(str(train_ppv)+'\n')
f.close()

filename3='/results/Extended Data Fig 10/EPE_clinic_16/auc/train.AUC_CI.txt'
f = open(filename3,'a')
...
filename4='/results/Extended Data Fig 10/EPE_clinic_16/ppv/val.PPV_CI.csv'
f = open(filename4,'a')
f.write(str(val_ppv)+'\n')
f.close()

filename5='/results/Extended Data Fig 10/EPE_clinic_16/auc/val.AUC_CI.txt'
f = open(filename5,'a')
...
filename6='/results/Extended Data Fig 10/EPE_clinic_16/ppv/test1.PPV_CI.csv'
f = open(filename6,'a')
f.write(str(test1_ppv)+'\n')
f.close()

filename7='/results/Extended Data Fig 10/EPE_clinic_16/auc/test1.AUC_CI.txt'
f = open(filename7,'a')
...
filename8='/results/Extended Data Fig 10/EPE_clinic_16/ppv/test2.PPV_CI.csv'
f = open(filename8,'a')
f.write(str(test2_ppv)+'\n')
f.close()

filename9='/results/Extended Data Fig 10/EPE_clinic_16/auc/test2.AUC_CI.txt'
f = open(filename9,'a')

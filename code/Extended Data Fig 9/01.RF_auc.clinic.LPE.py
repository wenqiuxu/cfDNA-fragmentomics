...
#input feature
gene_list_file = pd.read_table("/data/Extended Data Fig 9/RF.clinic.features.txt", index_col=0, header = None)
gene_list = gene_list_file.index.tolist()

#input matrix
train_disease = pd.read_table("/data/Extended Data Fig 9/clinic.H1.late_PE.txt", index_col=0).dropna()
val_disease = pd.read_table("/data/Extended Data Fig 9/clinic.H2.late_PE.txt", index_col=0).dropna()
test1_disease = pd.read_table("/data/Extended Data Fig 9/clinic.H3.late_PE.txt", index_col=0).dropna()
test2_disease = pd.read_table("/data/Extended Data Fig 9/clinic.H4.late_PE.txt", index_col=0).dropna()

train_control = pd.read_table("/data/Extended Data Fig 9/clinic.H1.control.txt", index_col=0).dropna()
val_control = pd.read_table("/data/Extended Data Fig 9/clinic.H2.control.txt", index_col=0).dropna()
test1_control = pd.read_table("/data/Extended Data Fig 9/clinic.H3.control.txt", index_col=0).dropna()
test2_control = pd.read_table("/data/Extended Data Fig 9/clinic.H4.control.txt", index_col=0).dropna()
...
final_predit = {'criterion': 'entropy', 'max_depth': 5, 'min_samples_leaf': 50, 'n_estimators': 200}
...
f = open ('/results/Extended Data Fig 9/model_clinic.LPE.dat','wb')
pickle.dump(clf,f)
f.close()

###plot AUC
filename1='/results/Extended Data Fig 9/AUC_clinic.LPE.pdf'
#画图
plt.figure()
lw = 2
